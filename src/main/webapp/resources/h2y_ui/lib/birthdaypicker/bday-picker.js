  ...
  $.fn.birthdaypicker = function( options ) {
    ...
    return this.each(function() {
      ...
      if (settings["placeholder"]) {
        $("<option value='0'>请选择</option>").appendTo($year);
        $("<option value='0'>请选择</option>").appendTo($month);
        $("<option value='0'>请选择</option>").appendTo($day);
      }
      ...
      for (var k=1; k<10; k++) { $("<option></option>").attr("value",k).text( "0" + k).appendTo($day); }
      for (var k=10; k<32; k++) { $("<option></option>").attr("value", k).text(k).appendTo($day); }
